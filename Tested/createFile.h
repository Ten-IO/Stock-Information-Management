...
void writeToFile(const Item &item, const std::string &filename){
...
// void writeToLogFile(const User &user, const std::string &filename){
//     std::ofstream file(filename, std::ios::app);
//     if (file.is_open()){
//         file << item.id << "," << item.name << "," << item.category << "," << item.units << "," << item.unitPrice << "\n";
//         file.close();
...
//username, product id, product name, action, qty, data, time
void writeLog(const string& user, int id, const string& name,const string& activity, int qty ) {
